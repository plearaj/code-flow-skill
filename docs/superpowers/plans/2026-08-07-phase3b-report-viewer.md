...
- **That the error card is visible.** The final review found that the call site itself was unpinned: `tests/test_template_contracts.py` asserted what `validate()` returns and `test/viewer-validation.test.js` asserted the same in isolation, but nothing checked that a failed verdict was actually wired to `fail()` — mutating the call site to `if (false) { fail(v.title, v.lines); return; }` left both suites green. `tests/test_template_contracts.py::test_scaffold_wires_a_failed_validation_to_fail` now pins that wiring (both scaffolds, mutation-confirmed), so that specific gap is closed. What remains unverified is narrower: `fail()`'s own body — `errBox.hidden = false`, clearing and rebuilding the error card's DOM — is still asserted for nothing. Deleting `errBox.hidden = false` from `fail()` would still leave the suite green.
